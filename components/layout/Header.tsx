...
import { Bell, Sun, Moon, Search, CheckCheck, LogOut, X } from 'lucide-react'
...
export default function Header({ title, subtitle }: HeaderProps) {
  ...
  return (
    <>
      {/* ── Header ───────────────────────────────────────────────── */}
      <header
        className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-30"
        ...
        <div className="h-14 md:h-16 flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
            ...
            <button
              ...
              className="w-9 h-9 flex items-center justify-center rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 active:bg-slate-100 dark:active:bg-slate-800 transition-colors"
            >
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="relative w-9 h-9 flex items-center justify-center rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 active:bg-slate-100 dark:active:bg-slate-800 transition-colors"
              >
                <Bell size={18} />
                {unreadCount > 0 && (
              ...
              {/* Panel de notificaciones — en móvil ocupa todo el ancho */}
              {notifOpen && (
                <div className={cn(
                  'absolute top-11 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl z-50 overflow-hidden',
                  // En móvil: alineado a la derecha con ancho máximo calculado para no salirse
                  'right-0 w-[calc(100vw-32px)] max-w-xs md:w-80'
                )}>
                  {/* Cabecera */}
                  <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 md:hidden"
                      >
                        <X size={13} />
                      </button>
                    </div>
                      ...
                      userNotifications
                        ...
                        .map(notif => (
                          <button
                            ...
                            className={cn(
                              'w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 active:bg-slate-50 dark:active:bg-slate-700/50 transition-colors',
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={cn(
                                ...
                                notif.leida ? 'text-slate-600 dark:text-slate-300' : 'text-slate-900 dark:text-white'
                              )}>
                                {notif.titulo}
                              </p>
            ...
            <button
              ...
              title="Cerrar sesión"
              className="w-9 h-9 flex items-center justify-center rounded-xl text-slate-500 dark:text-slate-400 hover:bg-red-50 dark:hover:bg-red-900/20 active:bg-red-50 dark:active:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 active:text-red-600 transition-colors"
            >
              <LogOut size={18} />
            </button>
          </div>
